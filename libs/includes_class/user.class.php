...
//here the login can be performed on both with the user name as well as id
    public function __construct($username)
    {
        ...
        $this->id = null;
        $sql = "SELECT * FROM `studlogin` WHERE `email` = '$username' OR `id`= '$username' LIMIT 1";
    ...
    public static function signup($email,$pass)
    {
        ...
        $sql = " INSERT INTO `studlogin` ( `email`,`password`)
    VALUES ('$email','$pass');";
        $result = true;
    ...
    public static function login($user, $pass)
    {
        ...
        $quer = "SELECT * FROM `studlogin` WHERE `email` = '$user'";
